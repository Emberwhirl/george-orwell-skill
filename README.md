...
Patch release for [issue #1](https://github.com/Emberwhirl/george-orwell-skill/issues/1).

- Pastiche label: the template no longer contains a literal `[current year]` placeholder. The skill is told to write the actual four-digit year.
- Timeline: Motihari is placed in Bengal Presidency, now Bihar. The *Homage to Catalonia* sales figure is hedged and sourced, matching `references/research/06-timeline.md`.
- Provenance: the unverifiable "174 sources" count is replaced with a pointer to `references/catalog.md` and `references/attribution/`.

### v0.3.1

Release v0.3.1: checkpoints for pastiche and writing workshop.
